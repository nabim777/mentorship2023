...
Given(
  "the user has created a new file {string} with content {string}",
...
When(
  "the user creates a new file {string} with content {string}",
  async function (fileName, content) {
    await entityPage.createFile(fileName, content);
  }
);
...
Then("the user should be able to see {string} file", async function (fileName) {
  const fileLocator = page.locator(
    util.format(entityPage.fileNameSelector, fileName)
  );
